...
int main() {
  ...
  int numbers[EDGE][EDGE] = {{8, 2, 22, 97, 38, 15, 00, 40, 00, 75,
  ...
  for (int i = 0; i < EDGE; i++) {
    for (int j = 0; j < EDGE - window; j++) {
      current = numbers[i][j] * numbers[i][j + 1] * numbers[i][j + 2] *
                numbers[i][j + 3];
      if (current > solution) {
        solution = current;
      }
  ...
  // vertical
  for (int i = 0; i < EDGE - window; i++) {
    for (int j = 0; j < EDGE; j++) {
      current = numbers[i][j] * numbers[i + 1][j] * numbers[i + 2][j] *
                numbers[i + 3][j];
      if (current > solution) {
        solution = current;
      }
  ...
  for (int i = 0; i < EDGE - window; i++) {
    for (int j = 0; j < EDGE - window; j++) {
      // NW
      current = numbers[i][j] * numbers[i + 1][j + 1] * numbers[i + 2][j + 2] *
                numbers[i + 3][j + 3];
      if (current > solution) {
        solution = current;
      }
      ...
      // NE
      current = numbers[i + 3][j] * numbers[i + 2][j + 1] *
                numbers[i + 1][j + 2] * numbers[i][j + 3];
      if (current > solution) {
        solution = current;
      }
    }
